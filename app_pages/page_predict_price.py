...
def page_predict_price_body():
    ...
    st.write("State Business Requirement 2")
    ...
    if st.button("Load and Predict Inherited Houses"):
        ...
        required_columns = ['OverallQual', 'GrLivArea', 'YearBuilt', 'TotalBsmtSF', 'GarageArea']
        if all(column in inherited_houses_df.columns for column in required_columns):
            predictions = model.predict(inherited_houses_df[required_columns])
